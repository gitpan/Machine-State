...
sub BUILDMACHINE {
    ...
    my $root  = Machine::State::State->new(name => $init);
    ...
    for my $node ($nodes->list) {
        my ($name, %args) = @{$node};
        my $state = $register{$name} //= Machine::State::State->new(
            name => $name
        );
    }
...
using L<Machine::State>. L<Machine::State> allows you to define a process, model
...
modeled using Machine::State::Simple. This library is a Moose-based
